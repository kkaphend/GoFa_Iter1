    ...
    if len(data) != 0:
        ...
        for i in range(len(data)):
            ...
            frames.append(Frame(point = Point(d["point"][0] + x_offset, d["point"][1] + y_offset, d["point"][2] + z_offset), 
                                xaxis = Vector(d["xaxis"][0], d["xaxis"][1], d["xaxis"][2]), 
...
def print_process():
    ...
    abb.send_and_wait(rrc.MoveToJoints(start_pos, [], travel_speed, rrc.Zone.FINE))
    ...
    # --- Turn on tool
    # extr = rrc.SetDigital('toolSand', 0)  
    # abb.send(extr)
    ...
    for i in range(len(frames)):
        if start_i <= i <= end_i:
            ...
            instruction = rrc.MoveToFrame(frames[i], print_speed, zone=rrc.Zone.Z1, motion_type=rrc.Motion.LINEAR)
    ...
    # --- turn off tool
    # extr = rrc.SetDigital('doA062_T1MotOn', 1)  
